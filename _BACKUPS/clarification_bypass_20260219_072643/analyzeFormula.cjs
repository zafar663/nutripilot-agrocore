...
function analyzeFormula({
  ...
  if (parsed.needs_clarification && parsed.needs_clarification.length > 0) {
    const clarification_text = buildClarificationText(parsed.needs_clarification);
    const clarification_examples = buildClarificationExamples(parsed.needs_clarification);
    ...
    return {
      ...
      message:
        "Some ingredients require CP/grade clarification. Please reply with the requested grades (examples included).",
    };
  }
  ...
  let req;
  try {
    req = resolveRequirements({ species, type, breed, phase, region });
  } catch (e) {
    return {
      ok: false,
