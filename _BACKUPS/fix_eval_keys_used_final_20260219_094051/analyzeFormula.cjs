...
function analyzeFormula({
  ...
  let _production =
    String(((typeof production !== "undefined" && production) || (enterprise && (enterprise.production || enterprise.production_type || enterprise.productionType || enterprise.category)) || "")).trim().toLowerCase() ||
    String(((enterprise && (enterprise.production || enterprise.production_type || enterprise.productionType || enterprise.category)) || "")).trim().toLowerCase();
...
console.log("[DBG req result]", { ok:req?.ok, error:req?.error, message:req?.message, reqKey:req?.reqKey, selectors:req?.selectors, details:req?.details, source:req?.source, reqFilePath:req?.reqFilePath });
  } catch (e) {
    return {
      ok: false,
...
const evaluation_keys_seed_tmp =
  (evaluation_keys_from_profile && evaluation_keys_from_profile.length > 0)
    ? evaluation_keys_from_profile
    : Object.keys(profileReqs_raw || {});
  ...
  const evaluation_keys_seed_tmp = keysWanted.filter((k) => coverage[k] && coverage[k].supported);
