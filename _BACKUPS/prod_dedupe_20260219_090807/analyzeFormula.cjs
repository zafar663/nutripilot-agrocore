...
function analyzeFormula({
  ...
  let _production =
    String((production || "")).trim().toLowerCase() ||
  ...
  try {
    ...
    if (!production && String(species||'').toLowerCase()==='poultry') {
      ...
      if (['duck','goose','quail','turkey'].includes(t)) {
        ...
        // ---- Option-B requirements: infer production + force v1 pass-through ----
    const _version = String(version || "v1").trim();
    let _production = String((typeof production !== "undefined" ? production : "") || "").trim().toLowerCase();
    let _production_inferred = false;
    ...
    console.log("[DBG req selectors]", { species, type, breed, phase, region, version_in: version, _version, _production, _production_inferred });
...
console.log("[DBG req result]", { ok:req?.ok, error:req?.error, message:req?.message, reqKey:req?.reqKey, selectors:req?.selectors, details:req?.details, source:req?.source, reqFilePath:req?.reqFilePath });
  ...
  const profileReqs_raw = req.profile && req.profile.targets ? req.profile.targets : {};
