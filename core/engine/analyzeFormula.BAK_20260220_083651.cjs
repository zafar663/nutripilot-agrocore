 ...
 *
 * NEW (SAFE):
 * - Alias + snake_case fallback resolution before ingredient DB lookup
 *   (fixes "l lys hcl" vs "l_lys_hcl", "soy oil" variants, etc.)
 *
 * NEW (SAFE DEBUG):
 * - meta.resolution_map: shows how each parsed ingredient resolves to DB key + hit/miss
 */
...
// ---- Aliases (optional). If missing, we still work. ----
// NOTE: This path is relative to core/engine/ -> core/db/aliases/...
...
/**
 * Resolve raw ingredient text into a DB key with trace.
 * - direct: raw matches a db key
 * - alias: normalized alias matches -> mapped key
 * - alias_snake: alias mapped, then snake form matches
 * - snake: snake(raw) matches
 * - none: returns raw (for visibility)
 */
function resolveDbKeyWithTrace(rawKey, aliases, db) {
  ...
  if (aliases && aliases[n]) {
    ...
    // return mapped anyway for debug visibility
...
function resolveDbKey(rawKey, aliases, db) {
  return resolveDbKeyWithTrace(rawKey, aliases, db).resolved;
}
...
function sumNutrients(itemsResolved, db, keysWanted, altMap, dm_ratio_map) {
  const out = {};
  ...
  for (const it of itemsResolved) {
    ...
    const dmRatio = (dm_ratio_map && dm_ratio_map[it.ingredient] != null)
      ? num(dm_ratio_map[it.ingredient])
      : 1;
...
/**
 * Dynamic theoretical corn maximum (% of diet) based on limiting nutrients.
 * Uses constant requirement comparison: max_pct = (required / corn_value) * 100
 * NOTE: This is a theoretical ceiling assuming corn is the only contributor for that nutrient,
 *       so it is an advisory metric, not a formulation result.
 */
function computeTheoreticalCornLimitPct({ db, profileReqs }) {
  if (!db || typeof db !== "object") return null;

  const corn = db.corn_dm_12 || db.corn || null;
  if (!corn || typeof corn !== "object") return null;

  const out = {};
  const keys = ["sid_lys", "avp", "na"];

  for (const k of keys) {
    const required = reqMin(profileReqs && profileReqs[k]);
    const cornVal = num(corn && corn[k]);

    if (required === null || !Number.isFinite(required) || required <= 0) continue;
    if (!Number.isFinite(cornVal) || cornVal <= 0) continue;

    out[k] = Math.min(100, +(((required / cornVal) * 100).toFixed(2)));
}

  return Object.keys(out).length ? out : null;
}
/**
 * Runtime moisture override (DM mass-balance) helpers
 * - All nutrients scale linearly with dry matter fraction (as-fed basis).
 * - DM fraction = 1 - moisture_pct/100
 * - ratio = DM_new / DM_base
 */
function computeDmRatioFromMoisture({ base_moisture_pct, new_moisture_pct }) {
  const b = Number(base_moisture_pct);
  const m = Number(new_moisture_pct);
  if (!Number.isFinite(b) || !Number.isFinite(m)) return null;
  if (b < 0 || b > 40) return null;
  if (m < 0 || m > 40) return null;

  const dmBase = 1 - (b / 100);
  const dmNew = 1 - (m / 100);
  if (dmBase <= 0 || dmNew <= 0) return null;

  return +(dmNew / dmBase).toFixed(8);
}

function getUserCornMoistureOverridePct({ enterprise, performance, moisture_overrides }) {
  const ent = enterprise && typeof enterprise === "object" ? enterprise : {};
  const perf = performance && typeof performance === "object" ? performance : {};
  const top = moisture_overrides && typeof moisture_overrides === "object" ? moisture_overrides : {};

  const entMo = ent.moisture_overrides && typeof ent.moisture_overrides === "object" ? ent.moisture_overrides : {};
  const perfMo = perf.moisture_overrides && typeof perf.moisture_overrides === "object" ? perf.moisture_overrides : {};

  // Accept several keys; prefer enterprise.moisture_overrides
  const cand =
    entMo.corn
    ?? entMo.corn_moisture_pct
    ?? entMo.cornMoisturePct
    ?? top.corn
    ?? top.corn_moisture_pct
    ?? perfMo.corn
    ?? null;
...
function buildDmRatioMapForCorn({ itemsResolved, db, enterprise, performance, moisture_overrides }) {
  const ingMeta = (typeof tryLoadIngredientMeta === "function") ? tryLoadIngredientMeta() : null;

  // Baseline moisture:
  // 1) ingredient_metadata anchor if present
  // 2) corn record in DB if present
  // 3) default 11.4 (Brazilian Tables corn avg DM=88.6%)
  const baseMoist =
    ingMeta?.ingredient_anchors?.corn?.baseline_moisture_pct
      ?? (db?.corn_dm_12?.moisture_pct ?? db?.corn?.moisture_pct)
      ?? 11.4;

  const userMoist = getUserCornMoistureOverridePct({ enterprise, performance, moisture_overrides });
  if (userMoist == null) return { dm_ratio_map: null, dm_overrides_applied: null };

  const ratio = computeDmRatioFromMoisture({ base_moisture_pct: baseMoist, new_moisture_pct: userMoist });
  if (ratio == null) return { dm_ratio_map: null, dm_overrides_applied: null };

  // Apply only to corn family keys present in the resolved diet.
  // Covers: "corn" and any corn_dm_* keys if used.
  const dm_ratio_map = {};
  const applied = [];

  for (const it of (itemsResolved || [])) {
    const k = String(it?.ingredient || "");
    if (!k) continue;
    if (k === "corn" || k.startsWith("corn_dm_")) {
      dm_ratio_map[k] = ratio;
      applied.push({
        ingredient: k,
        baseline_moisture_pct: baseMoist,
        user_moisture_pct: userMoist,
        dm_ratio: ratio
      });
    }
  }
  ...
  return {
    dm_ratio_map: Object.keys(dm_ratio_map).length ? dm_ratio_map : null,
    dm_overrides_applied: applied.length ? applied : null
  };
}
// ---- main ----
function analyzeFormula({
  ...
  // e.g. layer_peak -> peak, breeder_peak -> peak, layer_early -> early
  ...
  // Ingredients DB (basis=sid for poultry path)
  ...
  // Unknown ingredients (alias + snake fallback) + Resolution map (debug)
  const unknown = [];
  const itemsResolved = [];
  ...
  if (gate.needed) {
    ...
    return {
      ...
      dm_overrides_applied: dm_overrides_applied,
theoretical_corn_limit_pct: theoretical_corn_limit_pct,
// NEW (safe debug)
        resolution_map,
      },
      parsed,
  ...
  if (req && req.ok && hasAnyEnterpriseSignal && typeof genEnterpriseTargetsV1 === "function") {
    try {
  ...
  } else if (req && req.ok && hasAnyEnterpriseSignal && typeof genEnterpriseTargetsV1 !== "function") {
    enterprise_targets = {
      ok: false,
      generator_id: "enterprise_targets_v1",
  ...
  const evaluation_keys_defined_by_profile = req && req.ok ? pickEvaluationKeys({
  ...
  // Compute
    // Runtime DM/moisture override (corn): scales ALL nutrients by DM ratio (as-fed mass-balance)
  let dm_ratio_map = null;
  let dm_overrides_applied = null;
  try {
    const dmCtx = (() => {
    const userMoist = getUserCornMoistureOverridePct({ enterprise, performance, moisture_overrides });
    // baseline moisture: metadata -> db -> default 11.4
    const ingMeta = (typeof tryLoadIngredientMeta === "function") ? tryLoadIngredientMeta() : null;
    const baseMoist =
      ingMeta?.ingredient_anchors?.corn?.baseline_moisture_pct
        ?? (db?.corn_dm_12?.moisture_pct ?? db?.corn?.moisture_pct)
        ?? 11.4;
    const ratio = (userMoist == null) ? null : computeDmRatioFromMoisture({ base_moisture_pct: baseMoist, new_moisture_pct: userMoist });

    const built = buildDmRatioMapForCorn({ itemsResolved, db, enterprise, performance, moisture_overrides });
    const applied_keys = built && built.dm_ratio_map ? Object.keys(built.dm_ratio_map) : [];
    ...
    return {
      ...
      ratio,
      applied_keys,
      dm_ratio_map: built?.dm_ratio_map ?? null,
      dm_overrides_applied: built?.dm_overrides_applied ?? null
    };
  })();dm_ratio_map = dmCtx && dmCtx.dm_ratio_map ? dmCtx.dm_ratio_map : null;
    dm_overrides_applied = dmCtx && dmCtx.dm_overrides_applied ? dmCtx.dm_overrides_applied : null;
  } catch (_) {
    dm_ratio_map = null;
    dm_overrides_applied = null;
  }
    // ---- DM/moisture override (deterministic wiring) ----
  let dm_ratio_map = null;
  let dm_overrides_applied = null;

  // read user override (% moisture as-fed)
  const userCornMoist = getUserCornMoistureOverridePct({ enterprise, performance, moisture_overrides });

  // baseline moisture: metadata -> db -> default (Brazilian Tables corn avg DM=88.6% => moisture 11.4%)
  const ingMeta2 = (typeof tryLoadIngredientMeta === "function") ? tryLoadIngredientMeta() : null;
  const baseCornMoist =
    ingMeta2?.ingredient_anchors?.corn?.baseline_moisture_pct
      ?? (db?.corn?.moisture_pct ?? 11.4);

  const cornRatio = (userCornMoist == null)
    ? null
    : computeDmRatioFromMoisture({ base_moisture_pct: baseCornMoist, new_moisture_pct: userCornMoist });

  if (cornRatio != null) {
    dm_ratio_map = { corn: cornRatio };
    dm_overrides_applied = [{
      ingredient: "corn",
      baseline_moisture_pct: baseCornMoist,
      user_moisture_pct: userCornMoist,
      dm_ratio: cornRatio
    }];
  }
  // ---- end DM override wiring ----
  const nutrient_profile_full = sumNutrients(itemsResolved, db, keysWanted, altMap, dm_ratio_map);
  ...
  const evaluation = req && req.ok
    ? evaluateAgainstRequirements(profileReqs, nutrient_profile_full, evaluation_keys_used)
    : { overall: "NO_REQUIREMENTS", findings: [] };

  
  const theoretical_corn_limit_pct = computeTheoreticalCornLimitPct({ db, profileReqs });
return {
    ok: true,
...
enterprise_targets: enterprise_targets || null,
    ...
    requirements_basis_note: req && req.ok
      ...
      : (
          `No requirements profile could be loaded for: ` +
          `${species} → ${type} → ${breed} → ${phase} (region=${region}, version=${version}, production=${_production}).`
        ),
