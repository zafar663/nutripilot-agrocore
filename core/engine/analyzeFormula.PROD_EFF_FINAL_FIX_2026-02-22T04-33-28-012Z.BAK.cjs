 ...
 *
 * Fixes included in this full replacement:
 * 1) ✅ Registry keys: supports BOTH formats:
 *    - registry.nutrients as ARRAY [{ key:"me", ...}, ...]
 *    - registry.nutrients as OBJECT { me:{...}, cp:{...}, ...}
 *    Prevents numeric keys "0..N" from polluting nutrient_profile_full.
 *
 * 2) ✅ Correct ingredients loader import:
 *    loadIngredients must come from ../db/ingredients/_index/LOAD_INGREDIENTS.cjs
 *
 * 3) ✅ DM override:
 *    params.lab.ingredient_overrides[dbKey].dm (86 or 0.86)
 *    - if DM override exists and options.dm_scale_mode missing -> NEEDS_DM_SCALE_MODE
 *    dm_scale_mode:
 *      - ME_ONLY (default) scales only ME
 *      - ALL_NUTRIENTS scales all keys except computed (deb)
 *
 * 4) ✅ CP override:
 *    params.lab.ingredient_overrides[dbKey].cp (44 or 0.44)
 *    - if CP override exists and options.cp_apply_mode missing -> NEEDS_CP_APPLY_MODE
 *    cp_apply_mode:
 *      - RECOMPUTE_SID_FROM_TOTAL (recommended):
 *          total_* scaled by cp_ratio; sid_* = total_* * digestibility coef
 *      - TOTAL_ONLY:
 *          total_* scaled; sid_* unchanged (from DB)
 *      - SCALE_SID_DIRECT (fallback, not recommended):
 *          sid_* scaled directly by cp_ratio
 *
 * NOTE (your rule):
 * - CP change should primarily change TOTAL AAs.
 * - SID must be recomputed from TOTAL using digestibility coefficients.
 * This is exactly what RECOMPUTE_SID_FROM_TOTAL does.
 */
...
function buildAllowedNutrientKeys(registry) {
  const base = new Set([
    "me", "cp", "ca", "avp", "na", "k", "cl", "deb",
    "lys", "met", "met_cys", "thr", "trp", "arg", "ile", "leu", "val",
    "sid_lys", "sid_met", "sid_metcys", "sid_thr", "sid_trp", "sid_arg", "sid_ile", "sid_leu", "sid_val",
    "total_lys", "total_met", "total_metcys", "total_thr", "total_trp", "total_arg", "total_ile", "total_leu", "total_val",
  ]);
...
function analyzeFormula(params = {}) {
  ...
  if (_phase_req.includes("_")) {
    ...
    const allowed = new Set([
      "prelay", "pre-lay", "early", "peak", "late", "starter", "grower", "finisher",
      "breeder", "layer", "meat", "prepeak", "postpeak", "post-peak", "parent",
    ]);
  ...
  try {
    ...
    // NP_PRODUCTION_DEFAULT_v1 (MINIMAL FIX)
    // Ensure production is never undefined (prevents path.join(undefined) crash)
    // - Use _production (from params.production) if present
    // - Else infer a safe default from type
    ...
    // Keep meta consistent (so meta.production is not blank)
    if (!_production) {
    ...
    req = resolveRequirements({
      ...
      region,
      version: _version,
      production: (_production || production_eff || ""), // ✅ critical: never pass undefined
    });
    ...
    if (req && req.ok) {
      ...
      profileReqs_raw =
        (req && req.requirements && typeof req.requirements === "object")
          ? req.requirements
          : (req?.profile?.targets ? req.profile.targets : {});
  ...
  // Prefer registry full list if present; else fall back
  ...
  return {
    ...
    // ✅ IMPORTANT: do NOT Object.assign into nutrient_profile_full (avoid mutation surprises)
