 ...
 *
 * Lys curve behavior:
 * - OPTIONAL: runs only when opts.lys_curve.enabled === true
 * - Layers: egg mass drives Lys; BW contributes only small maintenance term (optional)
 * - Broiler breeders: BW is important only in breeder mode
 *
 * Option B (your choice):
 * - Truly dynamic Lys requirement from performance (override profile).
 * - Therefore auto-fit is OFF by default. (User can explicitly turn it on.)
 */
...
/**
 * Default coefficients (V1 placeholders; can be overridden by opts.coefs)
 ...
 * - "egg_mass_plus_bw_maint"    : Lys mg/d = (egg term) + (BW maintenance small, optional)
 * - "breeder_egg_output_plus_bw": Lys mg/d = a*EggOutput(g/d) + b*BW(kg)  [BW required]
 *
 * IMPORTANT:
 * - auto_fit_to_static default = false (Option B: truly dynamic override)
 *   User can set true explicitly if they want fit-to-profile behavior.
 */
const DEFAULT_LYS_CURVE = {
  layer: {
    mode: "egg_mass_plus_bw_maint",

    // production coefficient (mg Lys per g egg mass)
    // choose a sane default; user can override later via opts.lys_curve.layer_egg_mg_per_g
    layer_egg_mg_per_g: 14.0,
...
function genEnterpriseTargetsV1(staticTargetsRaw, context, performance, opts = {}) {
  ...
  function isLysCurveEnabledInOpts() {
    return !!(opts && opts.lys_curve && typeof opts.lys_curve === "object" && opts.lys_curve.enabled === true);
  }

  function getStaticSidLysPctMaybe(staticTargets) {
    // robust: accept either sid_lys_pct (raw) or sid_lys (already normalized)
    const a = toNumberOrNull(staticTargets?.sid_lys_pct);
    if (isNum(a)) return a;
    const b = toNumberOrNull(staticTargets?.sid_lys);
    if (isNum(b)) return b;
    return null;
  }

  function computeSidLysMgDay() {
    // OPTIONAL: only compute curve when explicitly enabled
    if (!isLysCurveEnabledInOpts()) {
      trace.lys_curve = { enabled: false, reason: "not_opted_in" };
      return null;
    }

    const lysCurveUser = opts.lys_curve || {};
    ...
    if (mode === "egg_mass_plus_bw_maint") {
      ...
      // Option B default: auto-fit OFF unless explicitly set true
      const autoFit = cfg.auto_fit_to_static === true;

      // accept static % from either sid_lys_pct or sid_lys
      const staticPct = getStaticSidLysPctMaybe(staticTargetsRaw);

      let prod_mg_per_g = prod_mg_per_g_default;
      let fitInfo = { used: false, autoFit, staticPct };
      ...
      trace.lys_curve.note =
        "Lys mg/d = (egg_mass*mg_per_g) + (BW_kg*maint_mg_per_kg*scale); auto-fit optional";
  ...
  // ===== SID AA =====
  if (canModelAA) {
    // If curve enabled → use it; else fall back to legacy A + B*egg_mass
