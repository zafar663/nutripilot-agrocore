 ...
 *
 * Ingredient-level DM scaling (optional):
 * - Uses ing.dm_pct with ing.adjust_policy.dm_scale.enabled
 * - Mode:
 *   - ME_ONLY: only scales "me"
 *   - ALL_NUTRIENTS: scales all nutrient keys (except computed "deb")
 */
function sumNutrients(itemsResolved, db, keysWanted, altMap, options = {}) {
  ...
  for (const it of itemsResolved) {
    ...
    const dmPct = ing ? ing.dm_pct : null;
    ...
    for (const k of keysWanted) {
      ...
      // Apply DM scaling mode
      let dmKeyFactor = 1;
      if (dmKeyFactorBase !== 1) {
        if (dmMode === "ALL_NUTRIENTS") {
          // scale all nutrient keys except computed outputs
          dmKeyFactor = k === "deb" ? 1 : dmKeyFactorBase;
        } else {
          // default ME_ONLY
...
function analyzeFormula(params = {}) {
  // ✅ Request options live here (NOT in requirements doc)
...
// Effective DM scaling mode (default ME_ONLY when not provided)
const dm_scale_mode_effective =
  (params && params.options && typeof params.options === "object" && params.options.dm_scale_mode)
    ? String(params.options.dm_scale_mode).trim()
    : "ME_ONLY";
  ...
  try {
    ...
    return {
      ...
      unknown,
      meta: {
        dm_scale_mode_used: dm_scale_mode_effective,
        locale, species, type, breed, phase, region, version, normalize: !!normalize
      },
  ...
  // Compute (✅ pass request options into sumNutrients)
