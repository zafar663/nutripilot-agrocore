 ...
 * AgroCore v1.4.4+ (enterprise targets + performance gating) — STABILIZED + Minimal DM override
 *
 * Kept:
 ...
 *
 * MINIMAL DM OVERRIDE (RESTORED, SAFE):
 * - If parser provides it.dm_percent (e.g. "SBM 48 30 DM=90"), nutrient contribution from that ingredient
 *   is scaled by (dm_percent/100). No requirement shifting, no modeling.
 * - meta.dm_overrides_applied always returned.
 */
...
function normalizeRequirementTargets(targets) {
  ...
  for (const [k0, v] of Object.entries(t)) {
    ...
    // energy
    if (k === "me_kcal_per_kg" || k === "me_kcal_kg" || k === "me_kcalkg" || k === "me") {
      canon = "me";
    }
...
/**
 * Sum nutrients with minimal DM override:
 * - each item can carry dm_ratio (default 1)
 * - contribution scales by dm_ratio * inclusion_pct
 */
function sumNutrients(itemsResolved, db, keysWanted, altMap, options = {}) {
  ...
  // Ingredient-level DM scaling (minimal, no requirement shifting):
  // Only applies to selected DM-based keys. ME/CP remain unchanged.
  const DM_SCALE_KEYS = new Set([
    "ee","starch","sugar","ndf","adf","ash",
    "zn_mgkg","cu_mgkg","mn_mgkg","fe_mgkg","se_mgkg","i_mgkg",
    "choline_mgkg","riboflavin_mgkg","niacin_mgkg","pantothenic_acid_mgkg",
    "b6_mgkg","b12_mgkg","folate_mgkg","biotin_mgkg"
  ]);
  for (const it of itemsResolved) {
    const ing = db[it.ingredient];
    if (!ing) continue;
    ...
    // DEBUG (temporary): capture dm scale mode + apply flags for corn
    const __dmMode = (options && options.dm_scale_mode) ? String(options.dm_scale_mode).trim() : "ME_ONLY";
    ...
    for (const k of keysWanted) {
      ...
      // Per-key DM scaling (ingredient-level)
      let dmKeyFactor = 1;
      const dmPolicy = ing?.adjust_policy?.dm_scale;
      const dmPct = ing?.dm_pct;

      {
      const dmMode = __dmMode;
      const dmApplies = (dmMode === "ALL_NUTRIENTS") || (dmMode === "ME_ONLY" && k === "me");

      if (dmApplies && dmPolicy?.enabled === true && isNum(dmPct) && dmPct > 0) {
        const ref = isNum(dmPolicy.ref_dm_pct) && dmPolicy.ref_dm_pct > 0 ? dmPolicy.ref_dm_pct : 88;
        dmKeyFactor = dmPct / ref;
      }
    }
...
function needsPerformanceForEnterpriseLys({ type, enterprise, performance }) {
  ...
  if (!isNum(fi) || fi <= 0) {
    missing.push({ key: "feed_intake_g_per_d", label: "Feed intake (g/hen/day)", example: "110" });
  }
  ...
  if (type === "layer") {
    ...
    if (!isNum(eggMass) || eggMass <= 0) {
      missing.push({ key: "egg_mass_g_per_d", label: "Egg output (g/hen/day)", example: "45" });
    }
  }
  ...
  if (bwRequiredForThisMode) {
    if (!isNum(bw) || bw <= 0) {
      missing.push({ key: "body_weight_kg", label: "Body weight (kg)", example: "3.8" });
    }
  }

  if (!missing.length) return { needed: false };

  return { needed: true, missing, detected: { fi, egg_mass_g_per_d: eggMass, body_weight_kg: bw, mode } };
}
...
function buildPerformancePrompt({ type, missing }) {
  ...
  for (const m of missing) lines.push(`- ${m.label} (send as: ${m.key}=${m.example})`);
  lines.push("");
  if (type === "layer") {
    lines.push("Examples:");
...
function analyzeFormula({
  ...
  // Unknown ingredients + Resolution map (debug) + DM overrides applied
  const unknown = [];
  const itemsResolved = [];
  ...
  for (const it of parsed.items || []) {
    ...
    // minimal DM handling (from parser)
    const dmPct = toNumberOrNull(it.dm_percent);
    const dm_ratio =
      isNum(dmPct) && dmPct > 0 && dmPct <= 100
        ? +(dmPct / 100).toFixed(8)
        : 1;
  ...
  try {
    ...
    return {
      ...
      unknown,
      meta: {
    dm_scale_mode_used: (options && options.dm_scale_mode) ? String(options.dm_scale_mode) : null, locale, species, type, breed, phase, region, version, normalize: !!normalize },
  ...
  if (gate.needed) {
    ...
    return {
      ...
      version: "AgroCore v1.4.4 (clarification-gated; enterprise targets)",
      meta: {
    dm_scale_mode_used: (options && options.dm_scale_mode) ? String(options.dm_scale_mode) : null,
        locale,
        species,
        type,
        breed,
  ...
  if (req && req.ok && hasAnyEnterpriseSignal && typeof genEnterpriseTargetsV1 === "function") {
    try {
      ...
      const opts = { coefs: ent.coefs || undefined, lys_curve: lysCurveOpts };
  ...
  // ---- PATCH: compute full nutrient profile even when reqKey is null ----
  // If registry exists -> compute ALL registry keys.
  // Else if requirements define keys -> compute those.
  // Else fallback to a safe default set.
  const registryKeysAll = Array.isArray(registry?.nutrients)
    ? registry.nutrients.map(n => n && n.key).filter(Boolean)
    : [];

  const keysForProfile = registryKeysAll.length
    ? registryKeysAll
    : (keysWanted.length ? keysWanted : [
        "me","amen","ne","cp","starch","sugars","ee","linoleic","linolenic","oleic","sfa","ufa",
        "cf","ndf","adf","lignin","nsp_total","nsp_sol","nsp_insol","beta_glucans","arabinoxylans",
        "ca","p_total","avp","npp","dig_p","mg","s","na","k","cl","deb",
        "sid_lys","sid_met","sid_cys","sid_metcys","sid_thr","sid_trp","sid_arg","sid_ile","sid_val","sid_leu",
        "sid_his","sid_phe","sid_tyr","sid_gly","sid_ser","sid_pro",
        "total_lys","total_met","total_cys","total_metcys","total_thr","total_trp","total_arg","total_ile","total_val","total_leu",
        "total_his","total_phe","total_tyr","total_gly","total_ser","total_pro",
        "vit_a","vit_d3","vit_e","vit_k","vit_b1","vit_b2","vit_b6","vit_b12","niacin","pantothenic_acid","folic_acid","biotin","choline",
        "zn","mn","cu","fe","i","se"
      ]);


  // Compute
  const options = (req && req.options && typeof req.options === "object") ? req.options : {};
  const nutrient_profile_full = sumNutrients(itemsResolved, db, keysForProfile, altMap, options, opts);
  const coverage = buildCoverage(itemsResolved, db, keysForProfile, altMap);
  ...
  return {
    ...
    dm_scale_mode_used: (options && options.dm_scale_mode) ? String(options.dm_scale_mode) : null,
      ...
      // ✅ Minimal DM override, always defined
      dm_overrides_applied,

      theoretical_corn_limit_pct,
    ...
    nutrient_profile_full: Object.assign(nutrient_profile_full, { coverage, unknown }),
